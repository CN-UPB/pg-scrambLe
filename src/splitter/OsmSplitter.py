...
#from OSMUtilityFunctions import utility
import json
import pymongo
...
class osm_splitter():
    ...
    def __init__(self,utility = None):
        ...
        self.NSDs = []
        self.network_function_sets = [["1"], ["2"]]
    ...
    def handle_elan_links(self,virtual_link_elan, nsd_vl):
        vld_instance = Vld(virtual_link_elan.id, virtual_link_elan.name, virtual_link_elan.short_name,
        ...
        if len(self.utilityFunctions.list_vnffgd) is not 0:
            for i in range(len(self.NSDs)):
                ...
                for fg in self.utilityFunctions.list_vnffgd:
                    vnffgd_instance = Vnffgd(fg.id, fg.name, fg.short_name, fg.vendor, fg.description, fg.version, [], [])
